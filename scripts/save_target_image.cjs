...
// Vector definition matching output_transparent.png
...
async function run() {
  ...
  const dirs = ['client/public/images', 'public/images', 'attached_assets'];
  for (const d of dirs) {
    if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
  }

  // Write exact target file requested by user
  fs.writeFileSync('client/public/images/output_transparent.png', pngBuffer);
  fs.writeFileSync('public/images/output_transparent.png', pngBuffer);
  ...
  fs.writeFileSync('attached_assets/output_transparent.png', pngBuffer);
  ...
  console.log('Successfully saved to client/public/images/output_transparent.png');
  await browser.close();
}
