  ...
  const createMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      if (!response.ok) {
        ...
        const details = errJson.errors && Array.isArray(errJson.errors)
          ? " - " + errJson.errors.map((e: any) => `${e.path.join(".")}: ${e.message}`).join(", ")
          : "";
        throw new Error((errJson.message || "خطا در ایجاد اشتراک") + details);
      }
      return response.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        title: "❌ خطا در ایجاد اشتراک",
        description: error.message || "خطا در ایجاد اشتراک رخ داد",
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: any }) => {
      ...
      if (!response.ok) {
        ...
        const details = errJson.errors && Array.isArray(errJson.errors)
          ? " - " + errJson.errors.map((e: any) => `${e.path.join(".")}: ${e.message}`).join(", ")
          : "";
        throw new Error((errJson.message || "خطا در بروزرسانی اشتراک") + details);
      }
      return response.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "خطا در بروزرسانی اشتراک رخ داد",
        variant: "destructive",
      });
    },
  });
