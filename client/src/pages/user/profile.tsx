...
import { User, Save, LogOut, Crown, Ticket as TicketIcon, Check, RefreshCw, Sparkles, AlertCircle, ShoppingCart, Globe } from "lucide-react";
  ...
  return (
    <DashboardLayout title="اطلاعات کاربری">
      <div className="space-y-4" data-testid="profile-content">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2 pt-2.5 border-t border-slate-100 dark:border-zinc-800/80">
                          ...
                          <Button
                            ...
                            className="h-8 sm:h-8.5 px-3 sm:px-4 text-[11px] sm:text-xs font-bold rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white shadow-sm flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap active:scale-95 transition-all cursor-pointer"
        ...
        {/* Website Domain Entry Popup Dialog */}
        <Dialog open={domainModalOpen} onOpenChange={setDomainModalOpen}>
          <DialogContent className="w-[calc(100%-24px)] sm:w-full sm:max-w-md p-4 sm:p-6 font-sans rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-2xl" dir="rtl">
            <DialogHeader className="text-right pb-3 border-b border-slate-100 dark:border-zinc-800">
              ...
              <DialogDescription className="text-xs text-slate-500 dark:text-zinc-400 mt-1.5 leading-relaxed">
                لطفاً نام یا دامنه اصلی وب‌سایت خود را وارد نمایید. پس از پرداخت موفق، این دامنه در تنظیمات درگاه به عنوان دامنه مجاز ثبت خواهد شد.
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-3.5 py-3 text-right">
              <div className="space-y-1.5">
                <Label htmlFor="domain-input-profile" className="text-xs font-bold text-slate-800 dark:text-zinc-200 block text-right">
                  نام یا آدرس وب‌سایت (دامنه مجاز):
                </Label>
                <div className="relative flex items-center" dir="ltr">
                  <Globe className="absolute left-3 w-4 h-4 text-slate-400 pointer-events-none" />
                  <Input
                    id="domain-input-profile"
                    type="text"
