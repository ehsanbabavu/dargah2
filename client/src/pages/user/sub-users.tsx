...
export default function SubUserManagement() {
  ...
  const handleDelete = (user: UserWithSubscription) => {
      ...
      <div className="space-y-4" data-testid="sub-users-content">
        ...
        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent dir="rtl">
            <AlertDialogHeader>
              <AlertDialogTitle>حذف زیرمجموعه</AlertDialogTitle>
              <AlertDialogDescription>
                آیا از حذف زیرمجموعه «{userToDelete?.firstName} {userToDelete?.lastName}» ({userToDelete?.username || userToDelete?.phone}) اطمینان دارید؟ این عملیات غیرقابل بازگشت است.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="gap-2">
              <AlertDialogCancel 
                ...
                disabled={deleteSubUserMutation.isPending}
              >
                انصراف
              </AlertDialogCancel>
              <AlertDialogAction
                onClick={(e) => {
                  e.preventDefault();
                  if (userToDelete) {
                    deleteSubUserMutation.mutate(userToDelete.id);
                  }
                }}
                disabled={deleteSubUserMutation.isPending}
                className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              >
                {deleteSubUserMutation.isPending ? "در حال حذف..." : "حذف زیرمجموعه"}
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
